...
const AnimateOnScroll = ({ children, reappear, threshold = 0.5 }: Props) => {
  ...
  return (
    <>
      <div ref={containerRef} className={`transition duration-1000 ${isVisible ? "opacity-100 blur-none translate-y-0" : "opacity-0 blur-lg -translate-y-40"}  motion-reduce:transition-none motion-reduce:hover:transform-none`}>
        {children}
      </div>
    </>
  );
